...
4. 如果題目包含選擇題選項 (例如 A, B, C, D)，請務必在選項前換行，讓每個選項獨立成行。
5. 解答與解析中的所有標點符號，請嚴格使用「中文全形標點符號」(例如：，、。！？；：「」『』)。
6. 解答請「只輸出解析內容」，絕對不要包含「解答」、「答案是」、「解析：」等開頭或標題，直接給出解釋與過程即可。

請「僅」回傳一個 JSON 格式，包含以下欄位：
{
  "subject": "科目名稱 (例如：國文, 數學, 英文)",
  "question": "完整題目內容 (選項請務必換行排版)",
  "answer": "正確答案 (例如 A, B, C 或簡短數值，不要寫詳解)",
  "solution": "純粹的解析內容 (全形標點、無標題、無答案)",
  "needs_image": true/false // 如果題目包含圖表、幾何圖形或必須看圖才能作答，請設為 true；如果純文字就能作答，請設為 false。
}
`;
...
1. 選擇題選項必須換行獨立排版。
2. 解答必須全部使用「中文全形標點符號」。
3. 解答「只能有解析」，不能出現「解答：」、「答案：」等標題。
4. 正確答案請單獨填入 "answer" 欄位中。
